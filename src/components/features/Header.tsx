...
export default function Header() {
  return (
    <div>
      <div className="flex items-center py-5 px-6 border-b border-b-[#CBD5E1] gap-2 ">
        <svg
      ...
      <div className="flex flex-col border-b border-b-[#CBD5E1]">
        {options.map((option, index) => (
          <div key={index} className="flex py-5 px-6 gap-2 items-center group">
            <option.icon className="w-6 h-6 text-[#94A3B8] group-hover:text-[#4F46E5]" />
            <h3 className="text-[18px] leading-6 font-bold tracking-[-0.144px] text-[#1E293B]  cursor-pointer hover:text-[#4F46E5] transition-all duration-300">
              {option.title}
            </h3>
          </div>
        ))}
